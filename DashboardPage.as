string HandleBfDashboard(const string &in body)
{
    ...
    // 3D viewer panel
    h += "<section class='panel wide' id='viewerPanel'>";
    h += "<h2>3D Viewer</h2>";
    h += "<iframe id='viewerFrame' class='viewer-frame' src='about:blank' title='3D Viewer'></iframe>";
    h += "</section>";
...
string BfDashCSS()
{
    ...
    c += ".viewer-hint{color:#8b949e;font-size:0.75rem;margin-bottom:0.5rem}";
    c += ".viewer-frame{width:100%;height:70vh;min-height:520px;border:1px solid #30363d;border-radius:6px;background:#0d1117}";
...
string BfDashJS_Status()
{
    ...
    j += "var pollOk=false;";
    j += "var viewerFrameReady=false;";
    j += "var viewerFrameLoaded=false;";
    j += "var viewerLastVersionByUid={};";
    j += "var viewerUidToPort={};";
    j += "var viewerSyncBusy=false;";
    ...
    // Viewer iframe + initial-run sync
    j += "function viewerPost(msg){";
    j += "var fr=document.getElementById('viewerFrame');";
    j += "if(!fr||!fr.contentWindow)return;";
    j += "try{fr.contentWindow.postMessage(msg,'*');}catch(e){}";
    j += "}";
    j += "function ensureViewerFrame(){";
    j += "var fr=document.getElementById('viewerFrame');";
    j += "if(!fr)return;";
    j += "if(viewerFrameLoaded&&fr.src&&fr.src.indexOf('/viewer/single')!==-1)return;";
    j += "viewerFrameReady=false;";
    j += "viewerFrameLoaded=true;";
    j += "fr.src='/viewer/single';";
    j += "}";
    j += "window.addEventListener('message',function(ev){";
    j += "var msg=ev&&ev.data?ev.data:null;";
    j += "if(!msg||msg.type!=='tmviewer-ready')return;";
    j += "viewerFrameReady=true;";
    j += "});";
    j += "function fetchJson(url){";
    j += "return fetch(url).then(function(r){if(!r.ok)throw new Error('HTTP '+r.status);return r.json();});";
    j += "}";
    j += "function syncViewerInitialRuns(){";
    j += "if(viewerSyncBusy)return;";
    j += "ensureViewerFrame();";
    j += "if(!viewerFrameReady)return;";
    j += "if(!instances||instances.length===0){";
    j += "var oldKeys=Object.keys(viewerLastVersionByUid);";
    j += "for(var oi=0;oi<oldKeys.length;oi++){viewerPost({type:'tmviewer-remove-instance-run',instanceUid:oldKeys[oi]});delete viewerLastVersionByUid[oldKeys[oi]];delete viewerUidToPort[oldKeys[oi]];}";
    j += "return;}";
    j += "viewerSyncBusy=true;";
    j += "var metaReqs=[];";
    j += "for(var i=0;i<instances.length;i++){";
    j += "(function(inst){";
    j += "var url='http://localhost:'+inst.port+'/api/bf/initial-run-meta';";
    j += "metaReqs.push(fetchJson(url).then(function(d){return {ok:true,port:inst.port,data:d};}).catch(function(){return {ok:false,port:inst.port,data:null};}));";
    j += "})(instances[i]);}";
    j += "Promise.all(metaReqs).then(function(metaResults){";
    j += "var live={};";
    j += "var fullReqs=[];";
    j += "for(var mi=0;mi<metaResults.length;mi++){";
    j += "var mr=metaResults[mi];if(!mr.ok||!mr.data)continue;";
    j += "var uid=(mr.data.instanceUid===null||mr.data.instanceUid===undefined)?('port:'+mr.port):String(mr.data.instanceUid);";
    j += "if(!uid)uid='port:'+mr.port;";
    j += "live[uid]=true;viewerUidToPort[uid]=mr.port;";
    j += "if(!mr.data.hasRun){";
    j += "if(Object.prototype.hasOwnProperty.call(viewerLastVersionByUid,uid)){viewerPost({type:'tmviewer-remove-instance-run',instanceUid:uid});delete viewerLastVersionByUid[uid];}";
    j += "continue;}";
    j += "var ver=Number(mr.data.version)||0;";
    j += "if(Object.prototype.hasOwnProperty.call(viewerLastVersionByUid,uid)&&viewerLastVersionByUid[uid]===ver)continue;";
    j += "(function(uidRef,portRef,verRef){";
    j += "var fullUrl='http://localhost:'+portRef+'/api/bf/initial-run';";
    j += "fullReqs.push(fetchJson(fullUrl).then(function(full){return {ok:true,uid:uidRef,port:portRef,ver:verRef,data:full};}).catch(function(){return {ok:false,uid:uidRef};}));";
    j += "})(uid,mr.port,ver);";
    j += "}";
    j += "return Promise.all(fullReqs).then(function(fullResults){";
    j += "for(var fi=0;fi<fullResults.length;fi++){";
    j += "var fr=fullResults[fi];if(!fr.ok||!fr.data||!fr.data.hasRun)continue;";
    j += "var label='Initial :'+fr.port;";
    j += "var payload=fr.data;";
    j += "payload.instanceUid=fr.uid;";
    j += "payload.label=label;";
    j += "viewerPost({type:'tmviewer-upsert-instance-run',payload:payload});";
    j += "viewerLastVersionByUid[fr.uid]=fr.ver;";
    j += "}";
    j += "var known=Object.keys(viewerLastVersionByUid);";
    j += "for(var ki=0;ki<known.length;ki++){";
    j += "var k=known[ki];if(live[k])continue;";
    j += "viewerPost({type:'tmviewer-remove-instance-run',instanceUid:k});";
    j += "delete viewerLastVersionByUid[k];";
    j += "delete viewerUidToPort[k];";
    j += "}";
    j += "});";
    j += "}).catch(function(){}).finally(function(){viewerSyncBusy=false;});";
    j += "}";
    j += "setInterval(syncViewerInitialRuns,2000);";
    j += "ensureViewerFrame();";
