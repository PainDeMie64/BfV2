...
array<InitialRunSample@> g_initialRunWorking;
array<InitialRunSample@> g_initialRunLatest;
uint g_initialRunVersion = 0;
bool g_initialRunCaptureInitialized = false;
BFPhase g_initialRunPrevPhase = BFPhase::Initial;

void ViewerResetInitialRunState()
{
    g_initialRunWorking.Resize(0);
    g_initialRunLatest.Resize(0);
    g_initialRunCaptureInitialized = false;
}

void ViewerStartInitialRunCapture()
{
    g_initialRunWorking.Resize(0);
}

void ViewerFinalizeInitialRunCapture()
{
    int startIdx = -1;
    int endIdx = -1;
    for (uint i = 0; i < g_initialRunWorking.Length; i++)
    {
        if (g_initialRunWorking[i] is null) continue;
        if (startIdx < 0) startIdx = int(i);
        endIdx = int(i);
    }

    // Nothing captured in this pass; keep previous run untouched.
    if (startIdx < 0 || endIdx < startIdx)
        return;

    uint needed = uint(endIdx + 1);
    if (g_initialRunLatest.Length < needed)
        g_initialRunLatest.Resize(needed);

    // Range patch: keep [0..startIdx-1], replace [startIdx..endIdx], drop > endIdx.
    for (int i = startIdx; i <= endIdx; i++)
    {
        InitialRunSample@ sample = g_initialRunWorking[uint(i)];
        if (sample is null) continue;
        @g_initialRunLatest[uint(i)] = sample;
    }
    g_initialRunLatest.Resize(needed);
    g_initialRunVersion++;
}

void ViewerCaptureInitialRunSample(SimulationManager@ simManager)
{
    ...
    uint uidx = uint(idx);
    if (uidx >= g_initialRunWorking.Length)
    {
        g_initialRunWorking.Resize(uidx + 1);
    }

    InitialRunSample@ s = InitialRunSample();
    s.raceTimeMs = raceTime;
    s.position = simManager.Dyna.CurrentState.Location.Position;
    s.velocity = simManager.Dyna.CurrentState.LinearSpeed;
    s.rotation = quat(simManager.Dyna.CurrentState.Location.Rotation);
    s.angularVelocity = simManager.Dyna.CurrentState.AngularSpeed;
    @g_initialRunWorking[uidx] = s;
}

void ViewerCapturePhaseStep(SimulationManager@ simManager)
{
    if (!g_initialRunCaptureInitialized)
    {
        g_initialRunPrevPhase = info.Phase;
        g_initialRunCaptureInitialized = true;
        if (info.Phase == BFPhase::Initial)
            ViewerStartInitialRunCapture();
    }

    if (g_initialRunPrevPhase != info.Phase)
    {
        if (g_initialRunPrevPhase == BFPhase::Initial && info.Phase != BFPhase::Initial)
            ViewerFinalizeInitialRunCapture();
        if (info.Phase == BFPhase::Initial)
            ViewerStartInitialRunCapture();
        g_initialRunPrevPhase = info.Phase;
    }
    ...
    if (info.Phase == BFPhase::Initial)
        ViewerCaptureInitialRunSample(simManager);
}
...
int GetInputModEffectiveMinTime(InputModificationSettings @settings, uint settingsIndex)
{
    ...
    else if (algo.identifier == "advanced_basic")
    {
        int steerCount = int(GetVariableDouble("bf_adv_steer_modify_count" + varSuffix));
        int accelCount = int(GetVariableDouble("bf_adv_accel_modify_count" + varSuffix));
        int brakeCount = int(GetVariableDouble("bf_adv_brake_modify_count" + varSuffix));
        if (steerCount > 0)
            minTime = Math::Min(minTime, int(GetVariableDouble("bf_adv_steer_min_time" + varSuffix)));
        if (accelCount > 0)
            minTime = Math::Min(minTime, int(GetVariableDouble("bf_adv_accel_min_time" + varSuffix)));
        if (brakeCount > 0)
            minTime = Math::Min(minTime, int(GetVariableDouble("bf_adv_brake_min_time" + varSuffix)));
        return minTime;
    }
    else if (algo.identifier == "advanced_range")
    {
        int steerMinCount = int(GetVariableDouble("bf_advr_steer_min_input_count" + varSuffix));
        int steerMaxCount = int(GetVariableDouble("bf_advr_steer_max_input_count" + varSuffix));
        int accelMinCount = int(GetVariableDouble("bf_advr_accel_min_input_count" + varSuffix));
        ...
        int brakeMaxCount = int(GetVariableDouble("bf_advr_brake_max_input_count" + varSuffix));
        if (steerMinCount > 0 || steerMaxCount > 0)
            minTime = Math::Min(minTime, int(GetVariableDouble("bf_advr_steer_min_time" + varSuffix)));
        if (accelMinCount > 0 || accelMaxCount > 0)
            minTime = Math::Min(minTime, int(GetVariableDouble("bf_advr_accel_min_time" + varSuffix)));
        if (brakeMinCount > 0 || brakeMaxCount > 0)
            minTime = Math::Min(minTime, int(GetVariableDouble("bf_advr_brake_min_time" + varSuffix)));
        return minTime;
    }
    else if (algo.identifier == "smooth_steering")
    {
        return int(GetVariableDouble("don_bf_modify_steering_min_time" + varSuffix));
    }

    return settings.minInputsTime;
}
int ComputeLeastInputModMinTime()
{
    int least = 1000000000;
    for (uint i = 0; i < g_inputModSettings.Length; i++)
    {
        int slotMin = GetInputModEffectiveMinTime(g_inputModSettings[i], i);
        if (slotMin < least)
            least = slotMin;
    }
    return least;
}
int GetInputModLeastMinTime()
{
    return ComputeLeastInputModMinTime();
}
int inputCount = 0;
int minInputsTime = 0;
int maxInputsTime = 0;
...
void OnSimulationEnd(SimulationManager @simManager, SimulationResult result)
{
    if (!IsBfV2Active)
        return;
    if (g_initialRunCaptureInitialized && g_initialRunPrevPhase == BFPhase::Initial)
    {
        ViewerFinalizeInitialRunCapture();
    }
    array<uint> sortedIndices(restartInfos.Length);
