namespace Scripting
{
    ...
    float GetCarAngVelPitch(SimulationManager @sim) { return sim.Dyna.CurrentState.AngularSpeed.x; }
    float GetCarAngVelYaw(SimulationManager @sim) { return sim.Dyna.CurrentState.AngularSpeed.y; }
    float GetCarAngVelRoll(SimulationManager @sim) { return sim.Dyna.CurrentState.AngularSpeed.z; }
    ...
    FloatGetter @ParseFactor(const string &in input)
    {
        ...
        if (lower == "car.velocity.z" || lower == "car.vel.z")
            return GetCarVelZ;
        if (lower == "car.velocity.pitch" || lower == "car.vel.pitch")
            return GetCarAngVelPitch;
        if (lower == "car.velocity.yaw" || lower == "car.vel.yaw")
            return GetCarAngVelYaw;
        if (lower == "car.velocity.roll" || lower == "car.vel.roll")
            return GetCarAngVelRoll;
